...
//
// This mirrors the routes in ../server.ts but is designed to run as a single
// Vercel Serverless Function. All routes are prefixed with /api and the
// vercel.json rewrite forwards every /api/* request here, so Express matches
// on the original URL. server.ts is kept for local `npm run dev`.
// -----------------------------------------------------------------------------
...
function evaluateHardwareCompatibility(req: HardwareValidationRequest): HardwareValidationResponse {
  ...
  const isWood =
    door.includes("wood") || door.includes("veneer") || door.includes("timber") || door.includes("flush wood");
  ...
  if (isFireRated) {
    ...
    if (
      lock.includes("passage") ||
      lock.includes("roller catch") ||
      lock.includes("dummy") ||
      (lock.includes("privacy") && !lock.includes("mortise") && !lock.includes("grade 1") && !lock.includes("grade 2"))
    ) {
      status = "incompatible";
      ruleCode = "ERR-NO-POSITIVE-LATCH";
      details.push(
        "Fire-rated doors must have an active positive-latching latchbolt. Non-latching passage sets, dummy pulls, and roller catches fail inspection."
      );
      recommendations.push(
        'Specify a ULC fire-listed Grade 1 or 2 Cylindrical or Mortise lockset with a minimum 1/2" throw latchbolt.'
      );
    } else {
  ...
  if (isHeavyFireRated) {
    ...
    if (
      hinge.includes("plain") ||
      hinge.includes("residential") ||
      hinge.includes("brass plain") ||
      hinge.includes("lightweight")
    ) {
      status = "incompatible";
      ...
      recommendations.push(
        'Upgrade to heavy-weight 4.5"x4.5" steel ball-bearing hinges (e.g. Hager BB1279 / McKinney) or continuous geared Roton hinges.'
      );
      ...
      details.push(
        "Spring-loaded hinges on heavy 3-Hour 16ga steel doors can struggle with latch reliability and hydraulic draft resistance."
      );
      recommendations.push(
        "Pair heavy-duty ball bearing hinges with an adjustable commercial hydraulic door closer (LCN 4040XP or ASSA ABLOY)."
      );
    } else {
  ...
  if (isFireRated && isWood && (hinge.includes("concealed") || hinge.includes("soss") || hinge.includes("3d"))) {
    ...
    if (status !== "incompatible") status = "warning";
    details.push(
      "Concealed 3D hinges on 20-minute wood doors require certified manufacturer intumescent fire liner jackets."
    );
    recommendations.push(
      "Ensure pre-cut intumescent hinge liners (e.g., Tectus / SOSS fire kits) are factory-embedded during door machining."
    );
  }
  ...
  if (
    (door.includes("stair") ||
      req.location?.toLowerCase().includes("stair") ||
      req.location?.toLowerCase().includes("exit")) &&
    !lock.includes("panic") &&
    !lock.includes("exit") &&
    !lock.includes("crash")
  ) {
    ...
    details.push(
      "Designated stairwell exit enclosures and egress paths with high occupant loads typically mandate single-motion panic exit hardware."
    );
  ...
  const testedAssemblies = isFireRated
    ? isHeavyFireRated
      ? "UL 10C / ULC-S104 90-180 Min Certified Assembly"
      : "UL 10C 20-45 Min Positive Pressure Assembly"
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ["All selected components match manufacturer machining tolerances and ULC listings."],
...
app.post("/api/submit-quote", (req, res) => {
  ...
  const prefix =
    quoteData.sector === "high-rise" ? "JD-HR" : quoteData.sector === "commercial" ? "JD-COM" : "JD-RES";
  ...
  res.json({
    ...
    message:
      "Door schedule and project details received. An architectural door consultant will review your specifications and supply a formal takeoff within 1-2 business days.",
    summary: {
